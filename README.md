...
Ограничения Web WeChat (puppet-wechat):
...
Траблшутинг (официальные рекомендации puppet-wechat):
- Если не скачивается Chromium (например, в Китае), можно задать:
  `PUPPETEER_DOWNLOAD_HOST=https://registry.npmmirror.com/mirrors`
- Если ошибка “Could not find expected browser”, задайте путь к браузеру:
  `WECHATY_PUPPET_WECHAT_ENDPOINT=/usr/bin/chromium-browser`
- Чтобы отключить stealth‑режим puppeteer:
